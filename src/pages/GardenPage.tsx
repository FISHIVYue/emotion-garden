...
import { getPreviewPlantState } from '../utils/previewPlantState'
...
export function GardenPage() {
  ...
  const plantState = getPreviewPlantState(location.search) ?? latest?.plantState ?? 'calm'
  ...
  return (
    <div className={`page page--with-nav garden-page garden-page--${plantState}`}>
      ...
      <section className="garden-page__stage" aria-labelledby="plant-name"><div className="garden-page__orbit" /><PlantStateLayer state={plantState}/><PlantIllustration plantId={plant.id} state={plantState} variant="hero" /><div className="garden-page__label"><span>与你共生</span><h2 id="plant-name">{plant.name}</h2><small>{plant.species}</small></div></section>
