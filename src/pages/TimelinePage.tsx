...
function EntryDetail({ entry }: { entry: EmotionEntry }) {
  const time = new Intl.DateTimeFormat('zh-CN', { hour: '2-digit', minute: '2-digit' }).format(new Date(entry.createdAt))
  return <article className={`timeline-entry timeline-entry--${entry.plantState}`}><header><span>{optionLabel(weatherOptions,entry.weather)}</span><time dateTime={entry.createdAt}>{time}</time></header>{entry.emotions.length > 0 && <div className="timeline-entry__tags">{entry.emotions.map((tag) => <span key={tag}>{optionLabel(emotionOptions,tag)}</span>)}</div>}{entry.note && <p>{entry.note}</p>}<small>{plantStateCopy[entry.plantState]}</small></article>
}
  ...
  return (
    <div className="page page--with-nav timeline-page">
      ...
      <section className="timeline-detail" aria-live="polite">
        {selectedDate ? <><header><div><p className="eyebrow">DAY SPECIMEN</p><h2>{new Intl.DateTimeFormat('zh-CN',{month:'long',day:'numeric',weekday:'long'}).format(new Date(`${selectedDate}T00:00:00`))}</h2></div><button type="button" onClick={() => navigate('/timeline')} aria-label="关闭单日详情">×</button></header>{selectedEntries.length ? <div className="timeline-detail__entries">{selectedEntries.map((entry) => <EntryDetail key={entry.id} entry={entry}/>)}</div> : <div className="timeline-empty"><i/><p>这一天没有留下记录。</p><span>空白也是时间自然经过的样子。</span></div>}</> : entries.length === 0 ? <div className="timeline-empty timeline-empty--main"><i/><p>年轮还没有长出第一片叶子。</p><span>当你记录一次心里的天气，它会安静地出现在这里。</span><button type="button" className="secondary-button" onClick={() => navigate('/garden')}>回花园记录此刻</button></div> : <div className="timeline-hint"><span aria-hidden="true">↟</span><p>轻触有叶片的日期，看看那天留下的天气。</p></div>}
      </section>
      <BottomNavigation />
    </div>
  )
